...
(function () {
  ...
  var css = [
    '.bbsm-ov{position:fixed;left:0;right:0;top:0;bottom:0;background:rgba(15,23,42,.32);z-index:4000}',
    '.bbsm-ov[hidden]{display:none}',
    '.bbsm-wrap{position:relative;z-index:4002;width:100%;min-width:0;display:flex;align-items:center;gap:10px}',
    '.bbsm-wrap .app-search,.bbsm-wrap .topbar-search{flex:1 1 auto;min-width:0}',
    '.bbsm-close-btn{flex:0 0 auto;border:0;background:none;padding:0 2px;color:var(--tp);font-size:14px;font-weight:500;cursor:pointer;font-family:inherit;white-space:nowrap}',
    '.bbsm-close-btn[hidden]{display:none}',
    '.bbsm-wrap *{box-sizing:border-box}',
    '.bbsm-wrap .app-search,.bbsm-wrap .topbar-search{position:relative;border-color:#E8385A;border-radius:8px;background:#F5F7FA;overflow:hidden}',
    '.bbsm-wrap .app-search::before,.bbsm-wrap .topbar-search::before{display:none}',
    '.bbsm-wrap .app-search input,.bbsm-wrap .topbar-search input{background:#F5F7FA;padding-left:116px}',
    '.bbsm-ai-chip{position:absolute;left:8px;top:50%;height:26px;transform:translateY(-50%);padding:0 10px;border-radius:8px;background:#E8385A;color:#fff;font-size:12px;font-weight:700;display:inline-flex;align-items:center;gap:4px;z-index:2;pointer-events:none;white-space:nowrap}',
    '.bbsm-ai-chip[hidden]{display:none}',
    '.bbsm-ai-chip b{font-weight:700}',
    '.bbsm-panel{position:fixed;left:16px;right:16px;width:auto;max-width:390px;margin:0 auto;background:#fff;border:1px solid #E8385A;border-top:0;border-radius:0 0 8px 8px;box-shadow:0 20px 60px rgba(0,0,0,.18);padding:16px 16px 32px;z-index:4001;overflow-x:hidden;overflow-y:auto;-webkit-overflow-scrolling:touch;font-family:var(--font-sans);color:var(--tp)}',
    '.bbsm-panel[hidden]{display:none}',
    '.bbsm-section{width:100%;padding-bottom:20px}',
    '.bbsm-section+.bbsm-section{padding-top:18px;border-top:1px solid var(--border)}',
    '.bbsm-row-head{width:100%;display:flex;align-items:center;justify-content:space-between;margin-bottom:13px}',
    '.bbsm-title{font-size:14px;font-weight:700;line-height:14px;color:var(--tp)}',
    '.bbsm-count{display:inline-flex;align-items:center;justify-content:center;min-width:18px;height:18px;margin-left:4px;border-radius:50%;background:#F5F7FA;color:#E8385A;font-size:11px;font-weight:700;vertical-align:1px;font-family:var(--font-num);letter-spacing:-0.28px}',
    '.bbsm-clearall{border:0;background:none;color:var(--tm);font-size:12px;font-weight:500;cursor:pointer;font-family:inherit;padding:0}',
    '.bbsm-scope-all{width:100%;height:44px;border:1px solid var(--border);border-radius:var(--r-btn,12px);background:#fff;color:var(--ts);font-size:13px;display:flex;align-items:center;justify-content:center;gap:7px;margin-bottom:7px;cursor:pointer;font-family:inherit}',
    '.bbsm-scope-all.active{border-color:#E8385A;background:#F5F7FA;color:#E8385A;font-weight:700}',
    '.bbsm-count[hidden],.bbsm-clearall[hidden]{display:none;font-family:var(--font-num)}',
    '.bbsm-scope-grid{width:100%;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}',
    '.bbsm-scope{height:48px;border:1px solid var(--border);border-radius:var(--r-btn,12px);background:#fff;color:var(--ts);font-size:12px;font-weight:500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;cursor:pointer;font-family:inherit;white-space:nowrap}',
    '.bbsm-scope.active{border-color:#E8385A;background:#F5F7FA;color:#E8385A}',
    ...
    '.bbsm-hint{display:block;margin-top:10px;color:var(--tm);font-size:12px}',
    '.bbsm-empty{color:var(--tm);font-size:13px}',
    '.bbsm-recent-clip{width:100%;overflow:hidden}',
    '.bbsm-recent-strip{display:flex;gap:8px;max-width:100%;overflow-x:auto;overflow-y:hidden;padding-bottom:18px;margin-bottom:-18px;scrollbar-width:none;-ms-overflow-style:none;-webkit-overflow-scrolling:touch}',
    '.bbsm-recent-strip::-webkit-scrollbar{width:0;height:0;display:none;background:transparent}',
    '.bbsm-recent-chip{height:38px;min-width:96px;max-width:180px;padding:0 10px;border:1px solid var(--border);border-radius:8px;background:#fff;color:var(--ts);font-size:13px;font-weight:500;display:inline-flex;align-items:center;justify-content:space-between;gap:8px;flex:0 0 auto;font-family:inherit}',
    '.bbsm-recent-label{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer}',
    '.bbsm-recent-remove{width:18px;height:18px;border:0;border-radius:50%;background:#F5F7FA;color:var(--tm);font-size:11px;line-height:18px;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;flex:0 0 auto;padding:0;letter-spacing:-0.28px}',
    '.bbsm-recent-remove:hover{background:#F5F7FA;color:#E8385A}',
    '.bbsm-suggest-list{list-style:none;margin:0;padding:0;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));column-gap:10px;row-gap:11px;counter-reset:bbsm-count}',
    '.bbsm-suggest-list.is-collapsed li.is-extra{display:none}',
    '.bbsm-suggest-list li{display:grid;grid-template-columns:16px minmax(0,1fr);gap:0;align-items:center;font-size:12px;color:var(--ts);cursor:pointer;counter-increment:bbsm-count;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
    '.bbsm-suggest-list li::before{content:counter(bbsm-count);color:var(--ts);font-size:12px;font-weight:400}',
    '.bbsm-suggest-list li:hover{color:#E8385A}',
    '.bbsm-suggest-toggle{margin-top:13px;border:0;background:none;color:#E8385A;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;padding:0}',
    ...
    '.bbsm-viewed{width:74px;height:72px;border:1px solid var(--border);border-radius:8px;overflow:hidden;background:#F5F7FA;flex:0 0 auto;padding:0;cursor:pointer}',
    '.bbsm-viewed img{width:100%;height:100%;object-fit:cover;display:block}',
    '@media (max-width:420px){.bbsm-panel{left:12px;right:12px;padding-left:14px;padding-right:14px}.bbsm-scope-grid{gap:8px}.bbsm-scope{font-size:11px;height:48px;letter-spacing:-0.28px;border-radius:var(--r-btn,12px)}.bbsm-wrap .app-search input,.bbsm-wrap .topbar-search input{padding-left:112px}}'
  ].join('\n');
  ...
  function init() {
    var search = document.querySelector('.app-search, .topbar-search');
    ...
    barChip.className = 'bbsm-ai-chip';
    barChip.hidden = true;
    ...
    function positionPanel() {
      ...
      panel.style.maxHeight = Math.max(window.innerHeight - top - 4, 240) + 'px';
